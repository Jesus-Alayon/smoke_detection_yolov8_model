...
class RMTTDriver(object):
    ...
    def __init__(self):
        ...
        self.pubFrontCam = rospy.Publisher('front_cam/image_raw', Image, queue_size=1)
        self.pubFrontCamInfo = rospy.Publisher('front_cam/camera_info', CameraInfo, queue_size=10)


        rospy.Subscriber("automan", Empty, self.callBackAutoman, queue_size=1)
        rospy.Subscriber("takeoff", Empty, self.callBackTakeOff, queue_size=1)
        rospy.Subscriber("land", Empty, self.callBackLand, queue_size=1)
    ...
    def callBackAutoman(self, data):
        print("Automatic Route")
        ...
        if (self.drone_state=="LANDED"):
            ...
            self.drone.flight.set_speed(speed=0.5)
            self.drone.flight.rotate(angle = 90).wait_for_completed()
            self.drone.flight.up(distance=20).wait_for_completed()
            
            self.drone.flight.left(distance=250).wait_for_completed()
            self.drone.flight.backward(distance=150).wait_for_completed()
            self.drone.flight.forward(distance=150).wait_for_completed()
            self.drone.flight.backward(distance=150).wait_for_completed()
            self.drone.flight.forward(distance=150).wait_for_completed()

            self.drone.flight.left(distance=200).wait_for_completed()
            self.drone.flight.backward(distance=150).wait_for_completed()
            self.drone.flight.forward(distance=150).wait_for_completed()
            self.drone.flight.backward(distance=150).wait_for_completed()
            self.drone.flight.forward(distance=150).wait_for_completed()
            ...
            self.drone.flight.right(distance=200).wait_for_completed()
            self.drone.flight.rotate(angle = 90).wait_for_completed()
            self.drone.flight.forward(distance=500).wait_for_completed()
            self.drone.flight.rotate(angle = 90).wait_for_completed()

            self.drone.flight.forward(distance=200).wait_for_completed()
            self.drone.flight.backward(distance=200).wait_for_completed()
            self.drone.flight.forward(distance=200).wait_for_completed()
            self.drone.flight.backward(distance=200).wait_for_completed()
            self.drone.flight.backward(distance=100).wait_for_completed()
    ...
    def subBatteryInfo(self, battery_info):
        ...
        if (self.battery_state=="NA"):  # Not Available
            ...
            if (battery_soc>60):
                print("  HIGH  battery: {0}".format(battery_soc))

        if (self.battery_state=="HIGH"):  # Not Available
            if (battery_soc<60):
                self.battery_state = "MEDIUM"
                print("  MEDIUM  battery: {0}".format(battery_soc))
    ...
    def run(self):
        ...
        if (RMTTDriver.ACTIVE_FRONT_CAM):
            ...
            self.drone.camera.set_bitrate(5)  
