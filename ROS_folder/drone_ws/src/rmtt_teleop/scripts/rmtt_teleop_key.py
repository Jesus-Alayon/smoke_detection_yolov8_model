...
class PublishThread(threading.Thread):
    ...
    def stop(self):
        ...
        # Sends 0 values to drone
        self.update(0, 0, 0, 0, 0, 0)

        # Waits until update is finished
        self.join()

    def run(self):
        twist = Twist()

        while not self.done: # Stop is not true
            ...
            # Publish the values to the drone.
            self.publisher.publish(twist)

        # Publish stop message when thread exits. Stop drone.
        twist.linear.x = 0
        twist.linear.y = 0
        twist.linear.z = 0
        twist.angular.x = 0
...
# It only prints the current speed
def vels(speed, turn):
