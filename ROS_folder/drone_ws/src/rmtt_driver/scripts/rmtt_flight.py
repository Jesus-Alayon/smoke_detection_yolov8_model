...
def initialize_ros():
    ...
    automan_pub = rospy.Publisher('/automan', Empty, queue_size=1)
    
    # Create the message format for the publishers
    empty_msg = Empty()
    twist_msg = Twist()

    return land_pub, takeoff_pub, move_pub, automan_pub, empty_msg, twist_msg

# Function called to indicate the drone's movement with the keyboard
def key_pressed(key, twist_msg, move_pub, takeoff_pub, land_pub, automan_pub, empty_msg):
    ...
    if key == kb.KeyCode.from_char('1'):
        Auto = not Auto
        print("Auto is " + str(Auto))

        if Auto:
            print("Start sequence")
            automan_pub.publish(empty_msg)
...
def main():
    land_pub, takeoff_pub, move_pub, automan_pub, empty_msg, twist_msg = initialize_ros()
    print("Ready for instructions")
    global Auto
    Auto = False

    listener = kb.Listener(
        on_press=lambda key: key_pressed(key, twist_msg, move_pub, takeoff_pub, land_pub, automan_pub, empty_msg),
